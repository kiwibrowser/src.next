...
public class PersonalizeResults {
    public static void Execute(Tab tab) {
       ...
       if (tab != null && ContextUtils.getAppSharedPreferences().getBoolean("accept_cookie_consent", true) && (tab.getUrl().getSpec().startsWith("http://") || tab.getUrl().getSpec().startsWith("https://"))) {
          tab.getWebContents().evaluateJavaScript("!function(){const SelectorListeners=[{selector:"#qcCmpUi",fn:acceptViaAPIs},{selector:"#didomi-popup",fn:acceptViaAPIs},{selector:'.accept-cookies-button,#purch-gdpr-banner__accept-button,#bbccookies-continue-button,.user-action--accept,.consent-accept,.bcpConsentOKButton,.button.accept,#footer_tc_privacy_button,button[aria-label="Button to collapse the message"],.gdpr-form>.btn[value="Continue"],button[on^="tap:"][on$=".accept"],button[on^="tap:"][on$=".dismiss"],.js-cookies-button,.app-offer__close_js,.lg-cc__button_type_action',fn:function(elem){elem.click()}}];function acceptViaAPIs(){"function"==typeof window.__cmpui&&window.__cmpui("setAndSaveAllConsent",!0),"object"==typeof window.Didomi&&window.Didomi.setUserAgreeToAll()}function check(){for(let{fn:fn,selector:selector}of SelectorListeners)for(let element of window.document.querySelectorAll(selector))element.ready||(element.ready=!0,selector.call(element,element))}window.addEventListener("DOMContentLoaded",(()=>{let newelem=document.createElement("style"),observer=new MutationObserver(check);newelem.innerHTML=".qc-cmp-showing { visibility: hidden !important; } body.didomi-popup-open { overflow: auto !important; } #didomi-host { visibility: hidden !important; }",(document.head||document.body)?.appendChild(newelem),observer.observe(document.documentElement,{childList:!0,subtree:!0}),window.setTimeout((()=>observer.disconnect()),15e3),window.setTimeout(check,5)}),{once:!0}),window.setTimeout(check,5)}();", null);
       }
    }
